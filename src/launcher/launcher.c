...
void init_launcher_panel(void* p) {
  ...
  // check consistency
  if (launcher->list_apps == NULL) return;
...
int resize_launcher(void* obj) {
  ...
  for (l = launcher->list_icons; l; l = l->next) {
    ...
    if (launcherIcon->icon_size != icon_size || !launcherIcon->icon_original) {
      ...
      char* new_icon_path =
          icon_path(launcher, launcherIcon->icon_name, launcherIcon->icon_size);
      if (!new_icon_path) {
        // Draw a blank icon
        free_icon(launcherIcon->icon_original);
  ...
  if (panel_horizontal) {
    if (!count)
      ...
      int height = launcher->area.height - 2 * launcher->area.bg->border.width -
                   2 * launcher->area.paddingy;
      // here icons_per_column always higher than 0
      icons_per_column = (height + launcher->area.paddingx) /
                         (icon_size + launcher->area.paddingx);
      marging = height -
                (icons_per_column - 1) * (icon_size + launcher->area.paddingx) -
                icon_size;
      icons_per_row =
    ...
    if (!count)
      ...
      int width = launcher->area.width - 2 * launcher->area.bg->border.width -
                  2 * launcher->area.paddingy;
      // here icons_per_row always higher than 0
      icons_per_row = (width + launcher->area.paddingx) /
                      (icon_size + launcher->area.paddingx);
      marging = width -
                (icons_per_row - 1) * (icon_size + launcher->area.paddingx) -
                icon_size;
      icons_per_column = count / icons_per_row + (count % icons_per_row != 0);
      launcher->area.height =
          (2 * launcher->area.bg->border.width) +
  ...
  int i, posx, posy;
  int start =
      launcher->area.bg->border.width + launcher->area.paddingy + marging / 2;
  if (panel_horizontal) {
    posy = start;
...
void launcher_icon_on_change_layout(void* obj) {
  ...
  launcherIcon->area.posy =
      ((Area*)launcherIcon->area.parent)->posy + launcherIcon->y;
  launcherIcon->area.posx =
      ((Area*)launcherIcon->area.parent)->posx + launcherIcon->x;
}
...
int parse_dektop_line(char* line, char** key, char** value) {
  ...
  *key = line;
  ...
  if (!found) return 0;
  if (found && (strlen(*key) == 0 || strlen(*value) == 0)) return 0;
  return 1;
}
...
void expand_exec(DesktopEntry* entry, const char* path) {
  ...
  if (entry->exec) {
    char* exec2 =
        malloc(strlen(entry->exec) + (entry->name ? strlen(entry->name) : 1) +
...
IconTheme* load_theme(char* name) {
  ...
  file_name =
      g_build_filename(g_get_home_dir(), ".icons", name, "index.theme", NULL);
  if (!g_file_test(file_name, G_FILE_TEST_EXISTS)) {
    ...
    if (!g_file_test(file_name, G_FILE_TEST_EXISTS)) {
      g_free(file_name);
      file_name =
          g_build_filename("/usr/share/pixmaps", name, "index.theme", NULL);
      if (!g_file_test(file_name, G_FILE_TEST_EXISTS)) {
        g_free(file_name);
        file_name = NULL;
      }
  ...
  while (item != NULL) {
    ...
    WARN("Dir:%s Size=%d MinSize=%d MaxSize=%d Threshold=%d Type=%s "
        ...
        dir->type == ICON_DIR_TYPE_FIXED
            ? "Fixed"
            : dir->type == ICON_DIR_TYPE_SCALABLE
                  ? "Scalable"
  ...
  while (app != NULL) {
    ...
    if (entry.exec) {
      ...
      launcherIcon->icon_name =
          entry.icon ? strdup(entry.icon) : strdup(ICON_FALLBACK);
      launcherIcon->icon_size = 1;
      launcherIcon->icon_tooltip =
...
void launcher_load_themes(Launcher* launcher) {
  ...
  // Free the queue
  GSList* l;
  for (l = queue; l; l = l->next) free(l->data);
  g_slist_free(queue);
  for (l = queued; l; l = l->next) free(l->data);
  g_slist_free(queued);
}
...
char* icon_path(Launcher* launcher, const char* icon_name, int size) {
  ...
  for (ext = extensions; ext; ext = g_slist_next(ext)) {
    ...
    if (strlen(icon_name) > strlen(extension) &&
        strcmp(extension, icon_name + strlen(icon_name) - strlen(extension)) ==
            0) {
  ...
  // the theme must have a higher priority than having an exact size match, so
  // we will just use
  // the code that searches for the best size match (it will find the exact size
  // match if one exists)
  /*
  for (theme = launcher->list_themes; theme; theme = g_slist_next(theme)) {
    GSList *dir;
    for (dir = ((IconTheme*)theme->data)->list_directories; dir; dir =
  g_slist_next(dir)) {
    ...
    for (dir = ((IconTheme*)theme->data)->list_directories; dir;
      ...
      for (base = basenames; base; base = g_slist_next(base)) {
        ...
        for (ext = extensions; ext; ext = g_slist_next(ext)) {
          ...
          char* extension = (char*)ext->data;
          char* file_name =
              malloc(strlen(base_name) + strlen(theme_name) + strlen(dir_name) +
