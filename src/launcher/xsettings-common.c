...
XSettingsSetting* xsettings_setting_copy(XSettingsSetting* setting) {
  ...
  case XSETTINGS_TYPE_STRING:
    ...
    if (!result->data.v_string) goto err;
...
void xsettings_setting_free(XSettingsSetting* setting) {
  if (setting->type == XSETTINGS_TYPE_STRING) free(setting->data.v_string);

  if (setting->name) free(setting->name);
...
XSettingsResult xsettings_list_insert(XSettingsList** list,
  ...
  if (!node) return XSETTINGS_NO_MEM;
...
XSettingsSetting* xsettings_list_lookup(XSettingsList* list, const char* name) {
  ...
  iter = list;
  while (iter) {
    if (strcmp(name, iter->setting->name) == 0) return iter->setting;
