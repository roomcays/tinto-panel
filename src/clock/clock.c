...
int resize_clock(void* obj) {
  ...
  get_text_size2(time1_font_desc, &time_height_ink, &time_height, &time_width,
                 panel->area.height, panel->area.width, buf_time,
                 strlen(buf_time));
  if (time2_format) {
    ...
    get_text_size2(time2_font_desc, &date_height_ink, &date_height, &date_width,
                   panel->area.height, panel->area.width, buf_date,
                   strlen(buf_date));
  }
  ...
  if (panel_horizontal) {
    ...
    new_size +=
        (2 * clock->area.paddingxlr) + (2 * clock->area.bg->border.width);
    ...
    int new_size =
        time_height + date_height +
        (2 * (clock->area.paddingxlr + clock->area.bg->border.width));
