...
void get_snapshot(const char* path) {
  ...
  panel->temp_pmap =
      XCreatePixmap(server.dsp, server.root_win, panel->area.width,
...
Atom pick_target_from_atoms(Display* disp, Atom t1, Atom t2, Atom t3) {
  ...
  if (t1 != None) atoms[n++] = t1;

  if (t2 != None) atoms[n++] = t2;

  if (t3 != None) atoms[n++] = t3;
...
void dnd_enter(XClientMessageEvent* e) {
  ...
  if (more_than_3) {
    ...
    Property p =
        read_property(server.dsp, dnd_source_window, server.atom.XdndTypeList);
...
void dnd_position(XClientMessageEvent* e) {
  ...
  se.data.l[1] = accept ? 1 : 0;  // bit 0: accept drop    bit 1: send
  // XdndPosition events if inside rectangle
  se.data.l[2] = 0;  // Rectangle x,y for which no more XdndPosition events
  se.data.l[3] =
      (1 << 16) | 1;  // Rectangle w,h for which no more XdndPosition events
  if (accept) {
  ...
  while (1) {
        ...
        if (panel->hidden) {
          ...
          panel->temp_pmap =
              XCreatePixmap(server.dsp, server.root_win, panel->area.width,
                            panel->area.height, server.depth);
          rendering(panel);
          XCopyArea(server.dsp, panel->temp_pmap, panel->main_win, server.gc, 0,
                    0, panel->area.width, panel->area.height, 0, 0);
        }
      }
      XFlush(server.dsp);
    ...
    if (select(x11_fd + 1, &fdset, 0, 0, timeout) > 0) {
      while (XPending(server.dsp)) {
        ...
        if (panel && panel_autohide) {
          ...
          if (panel->hidden) {
            if (e.type == ClientMessage &&
              ...
              continue;  // discard further processing of this event because the
        ...
        case MotionNotify: {
          unsigned int button_mask = Button1Mask | Button2Mask | Button3Mask |
                                     Button4Mask | Button5Mask;
          if (e.xmotion.state & button_mask) event_button_motion_notify(&e);
        ...
        case DestroyNotify:
          ...
          if (e.xany.window == g_tooltip.window || !systray_enabled) break;
        ...
        case SelectionNotify: {
          ...
          if (e.xselection.property != None && dnd_launcher_exec) {
            Property prop =
                read_property(server.dsp, dnd_target_window, dnd_selection);
              ...
              for (i = 0; i < prop.nitems * prop.format / 8; i++) {
                ...
                if (c == '\n') {
                  if (i < prop.nitems * prop.format / 8 - 1) {
                    cmd_length +=
                        3;  // close double quotes, space, open double quotes
                  }
                } else if (c == '\r') {
                } else {
