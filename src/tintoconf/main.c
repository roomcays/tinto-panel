...
static gboolean searchTheme(const gchar* name_theme, GtkTreeModel* model,
                            GtkTreeIter* iter);
...
static GtkActionEntry entries[] = {
    ...
    {"ThemeAdd", GTK_STOCK_ADD, _("_Add..."), "<Control>N", _("Add theme"),
     G_CALLBACK(menuAdd)},
    {"ThemeSaveAs", GTK_STOCK_SAVE_AS, _("_Save as..."), NULL,
     _("Save theme as"), G_CALLBACK(menuSaveAs)},
    {"ThemeDelete", GTK_STOCK_DELETE, _("_Delete"), NULL, _("Delete theme"),
     G_CALLBACK(menuDelete)},
    {"ThemeProperties", GTK_STOCK_PROPERTIES, _("_Properties..."), NULL,
     _("Show properties"), G_CALLBACK(menuProperties)},
    {"ThemeQuit", GTK_STOCK_QUIT, _("_Quit"), "<control>Q", _("Quit"),
     G_CALLBACK(menuQuit)},
    {"EditMenu", NULL, "Edit", NULL, NULL, NULL},
    {"EditRefresh", GTK_STOCK_REFRESH, _("Refresh"), NULL, _("Refresh"),
     G_CALLBACK(menuRefresh)},
    {"EditRefreshAll", GTK_STOCK_REFRESH, _("Refresh all"), NULL,
     _("Refresh all"), G_CALLBACK(menuRefreshAll)},
    //	{"EditPreferences", GTK_STOCK_PREFERENCES, "Preferences", NULL,
    //"Preferences", G_CALLBACK (menuPreferences)},
    {"ViewApply", GTK_STOCK_APPLY, _("Apply"), NULL, _("Apply theme"),
     G_CALLBACK(menuApply)},
    {"HelpMenu", NULL, _("Help"), NULL, NULL, NULL},
    {"HelpAbout", GTK_STOCK_ABOUT, _("_About"), "<Control>A", _("About"),
...
int main(int argc, char** argv) {
  ...
  g_signal_connect(g_window, "size-allocate", G_CALLBACK(windowSizeAllocated),
                   NULL);
  ...
  g_signal_connect(globalUIManager, "add_widget", G_CALLBACK(menuAddWidget),
                   vBox);
  ...
  g_signal_connect(g_theme_view, "button-press-event",
                   (GCallback)view_onButtonPressed, NULL);
  g_signal_connect(g_theme_view, "popup-menu", (GCallback)view_onPopupMenu,
                   NULL);
  g_signal_connect(g_theme_view, "row-activated", G_CALLBACK(viewRowActivated),
                   NULL);
...
static void menuAddWidget(GtkUIManager* p_uiManager, GtkWidget* p_widget,
                          GtkContainer* p_box) {
...
static void menuAbout() {
  ...
  gtk_show_about_dialog(
      GTK_WINDOW(g_window), "name", g_get_application_name(), "comments",
      _("Theming tool for tint2 panel"), "version", VERSION_STRING, "copyright",
      _("Copyright 2009 tint2 team\nTint2 License GNU GPL version "
...
static void menuAdd() {
  ...
  dialog = gtk_file_chooser_dialog_new(
      _("Add a theme"), GTK_WINDOW(g_window), GTK_FILE_CHOOSER_ACTION_OPEN,
      GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_ADD, GTK_RESPONSE_ACCEPT,
      NULL);
  ...
  for (l = list; l; l = l->next) {
    ...
    if (searchTheme(path, model, &iter)) {
      gchar* message;
      message =
          g_strdup_printf(_("Couldn't add duplicate theme\n\'%s\'."), pt1);

      GtkWidget* w =
          gtk_message_dialog_new(GTK_WINDOW(g_window), 0, GTK_MESSAGE_ERROR,
    ...
    if (name_first == NULL) name_first = g_strdup(path);
    g_free(path);
    g_free(name);
  }
...
static void menuSaveAs() {
  ...
  if (!gtk_tree_selection_get_selected(GTK_TREE_SELECTION(sel), &model,
                                       &iter)) {
    GtkWidget* w = gtk_message_dialog_new(GTK_WINDOW(g_window), 0,
                                          GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
                                          _("Select the theme to be saved."));
  ...
  dialog = gtk_file_chooser_dialog_new(
      _("Save theme as"), GTK_WINDOW(g_window), GTK_FILE_CHOOSER_ACTION_SAVE,
...
static void menuQuit() {
  ...
  if (g_path_config) g_free(g_path_config);
  if (g_path_dir) g_free(g_path_dir);
  if (g_default_theme) g_free(g_default_theme);
  if (g_cmd_property) g_free(g_cmd_property);
...
static void load_theme(GtkWidget* list) {
  ...
  if (!found_theme) {
    ...
    name = g_build_filename(g_get_user_config_dir(), "tint2", "default.tint2rc",
                            NULL);
...
void selectTheme(const gchar* name_theme) {
  ...
  if (found_theme)
    ...
    have_iter = gtk_tree_model_get_iter_first(model, &iter);
    if (have_iter) path = gtk_tree_model_get_path(model, &iter);
  }
  if (path) {
    gtk_tree_selection_select_iter(
        gtk_tree_view_get_selection(GTK_TREE_VIEW(g_theme_view)), &iter);
    gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(g_theme_view), path, NULL, FALSE,
                                 0, 0);
    gtk_tree_path_free(path);
  }
}

gboolean searchTheme(const gchar* name_theme, GtkTreeModel* model,
                     GtkTreeIter* iter) {
  gchar* name;
...
void initTheme() {
  ...
  if (!g_file_test(g_path_dir, G_FILE_TEST_IS_DIR)) g_mkdir(g_path_dir, 0777);
...
void read_config() {
  ...
  g_width = 500;
  g_height = 350;
  g_cmd_property = g_strconcat("/usr/bin/env python ", INSTALL_PREFIX,
                               "/bin/tintwizard.py", (void*)0);
