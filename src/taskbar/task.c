...
Task* add_task(Window win) {
  ...
  new_tsk.current_state =
      window_is_iconified(win) ? TASK_ICONIFIED : TASK_NORMAL;
  ...
  *key = new_tsk.win;
  ...
  if (window_is_urgent(win)) add_urgent(new_tsk2);
...
int get_title(Task* tsk) {
  ...
  name = server_get_property(tsk->win, server.atom._NET_WM_VISIBLE_NAME,
                             server.atom.UTF8_STRING, 0);
  if (!name || !strlen(name)) {
    name = server_get_property(tsk->win, server.atom._NET_WM_NAME,
                               server.atom.UTF8_STRING, 0);
    if (!name || !strlen(name)) {
...
void get_icon(Task* tsk) {
  ...
  data =
      server_get_property(tsk->win, server.atom._NET_WM_ICON, XA_CARDINAL, &i);
  if (data) {
    // get ARGB icon
    int w, h;
    gulong* tmp_data;

    tmp_data = get_best_icon(data, get_icon_count(data, i), i, &w, &h,
  ...
  for (k = 0; k < TASK_STATE_COUNT; ++k) {
    ...
    if (panel->g_task.alpha[k] != 100 || panel->g_task.saturation[k] != 0 ||
      ...
      adjust_asb(data32, tsk->icon_width, tsk->icon_height,
  ...
  if (task_group) {
    for (i = 0; i < task_group->len; ++i) {
      ...
      int k;
      for (k = 0; k < TASK_STATE_COUNT; ++k) tsk2->icon[k] = tsk->icon[k];
      set_task_redraw(tsk2);
    }
  }
}
...
void draw_task(void* obj, cairo_t* c) {
  ...
  if (panel->g_task.text) {
    ...
    pango_layout_set_width(
        layout, ((Taskbar*)tsk->area.parent)->text_width * PANGO_SCALE);
...
Task* find_active_task(Task* current_task, Task* active_task) {
  if (active_task == 0) return current_task;
  if (active_task->desktop != ALLDESKTOP) return active_task;
  if (current_task == 0) return active_task;
  ...
  for (; l0; l0 = l0->next) {
    tsk = l0->data;
    if (tsk->win == active_task->win) return tsk;
  }
  return active_task;
}
...
void active_task() {
  if (task_active) {
    set_task_state(task_active, window_is_iconified(task_active->win)
                                    ? TASK_ICONIFIED
                                    : TASK_NORMAL);
    task_active = 0;
  }
  ...
  if (w1) {
    if (!task_get_tasks(w1)) {
      Window w2;
      while (XGetTransientForHint(server.dsp, w1, &w2)) w1 = w2;
    }
...
void set_task_state(Task* tsk, int state) {
  if (tsk == 0 || state < 0 || state >= TASK_STATE_COUNT) return;
  ...
  if (tsk->current_state != state) {
    ...
    if (task_group) {
      ...
      for (i = 0; i < task_group->len; ++i) {
        ...
        if (tsk1->state_pix[state] == 0) tsk1->area.redraw = 1;
  ...
  while (urgent_task) {
    ...
    if (t->urgent_tick < max_tick_urgent) {
      if (t->urgent_tick++ % 2)
        ...
        set_task_state(
            t, window_is_iconified(t->win) ? TASK_ICONIFIED : TASK_NORMAL);
    }
...
void add_urgent(Task* tsk) {
  ...
  if (task_active && task_active->win == tsk->win) return;

  tsk = task_get_task(tsk->win);  // always add the first tsk for a task group
                                  // (omnipresent windows)
  tsk->urgent_tick = 0;
  if (g_slist_find(urgent_list, tsk)) return;
