...
/* win_to_task_table holds for every Window an array of tasks. Usually the array
   contains only one
   element. However for omnipresent windows (windows which are visible in every
   taskbar) the array
   contains to every Task* on each panel a pointer (i.e. GPtrArray.len ==
   server.nb_desktop)
*/
...
void init_taskbar() {
  if (win_to_task_table == 0)
    win_to_task_table =
        g_hash_table_new_full(win_hash, win_compare, free, free_ptr_array);
...
void init_taskbar_panel(void* p) {
  ...
  if (panel->g_taskbar.background[TASKBAR_NORMAL] == 0) {
    panel->g_taskbar.background[TASKBAR_NORMAL] =
        &g_array_index(backgrounds, Background, 0);
    panel->g_taskbar.background[TASKBAR_ACTIVE] =
        &g_array_index(backgrounds, Background, 0);
  }
  if (panel->g_taskbar.background_name[TASKBAR_NORMAL] == 0) {
    panel->g_taskbar.background_name[TASKBAR_NORMAL] =
        &g_array_index(backgrounds, Background, 0);
    panel->g_taskbar.background_name[TASKBAR_ACTIVE] =
        &g_array_index(backgrounds, Background, 0);
  }
  ...
  if (panel_horizontal) {
    panel->g_taskbar.area.posy =
        panel->area.bg->border.width + panel->area.paddingy;
    panel->g_taskbar.area.height =
        panel->area.height - (2 * panel->g_taskbar.area.posy);
    ...
    panel->g_taskbar.area.width =
        panel->area.width - (2 * panel->g_taskbar.area.posx);
  ...
  if ((panel->g_task.config_asb_mask & (1 << TASK_ACTIVE)) == 0) {
    ...
    panel->g_task.saturation[TASK_ACTIVE] =
  ...
  if ((panel->g_task.config_asb_mask & (1 << TASK_URGENT)) == 0) {
    ...
    panel->g_task.saturation[TASK_URGENT] =
        panel->g_task.saturation[TASK_ACTIVE];
  ...
  for (j = 0; j < TASK_STATE_COUNT; ++j) {
    ...
    if (panel->g_task.background[j]->border.rounded >
      ...
      panel->g_task.background[j] =
          &g_array_index(backgrounds, Background, backgrounds->len - 1);
      panel->g_task.background[j]->border.rounded =
          panel->g_task.area.height / 2;
    }
  }
  ...
  get_text_size(panel->g_task.font_desc, &height_ink, &height,
                panel->area.height, "TAjpg", 5);

  if (!panel->g_task.maximum_width && panel_horizontal)
    panel->g_task.maximum_width = server.monitor[panel->monitor].width;

  panel->g_task.text_posx =
      panel->g_task.background[0]->border.width + panel->g_task.area.paddingxlr;
  panel->g_task.text_height =
      panel->g_task.area.height - (2 * panel->g_task.area.paddingy);
  if (panel->g_task.icon) {
    panel->g_task.icon_size1 =
        panel->g_task.area.height - (2 * panel->g_task.area.paddingy);
    panel->g_task.text_posx += panel->g_task.icon_size1;
    panel->g_task.icon_posy =
        (panel->g_task.area.height - panel->g_task.icon_size1) / 2;
  }
...
void task_refresh_tasklist() {
  ...
  win = server_get_property(server.root_win, server.atom._NET_CLIENT_LIST,
  ...
  for (it = win_list; it; it = it->next) {
    for (i = 0; i < num_results; i++)
      if (*((Window*)it->data) == win[i]) break;
    if (i == num_results) taskbar_remove_task(it->data, 0, 0);
  }
...
int resize_taskbar(void* obj) {
  ...
  if (panel_horizontal) {
    ...
    taskbar->text_width = text_width - panel->g_task.text_posx -
                          panel->g_task.area.bg->border.width -
                          panel->g_task.area.paddingx;
  } else {
    resize_by_layout(obj, panel->g_task.maximum_height);

    taskbar->text_width =
        taskbar->area.width - (2 * panel->g_taskbar.area.paddingy) -
...
void set_taskbar_state(Taskbar* tskbar, int state) {
  ...
  if (tskbar->area.on_screen == 1) {
    if (tskbar->state_pix[state] == 0) tskbar->area.redraw = 1;
    ...
    if (panel_mode == MULTI_DESKTOP &&
      ...
      if (taskbarname_enabled) l = l->next;
      for (; l; l = l->next) set_task_redraw(l->data);
    }
  }
  panel_refresh = 1;
}
