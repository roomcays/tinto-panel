...
#if defined(__OpenBSD__) || defined(__NetBSD__)
  if ((apm_fd != -1) && (close(apm_fd) == -1)) warn("cannot close /dev/apm");
#endif
}
...
void init_battery() {
  ...
  if (error)
    ...
    while ((entryname = g_dir_read_name(directory))) {
      ...
      char* path1 = g_build_filename("/sys/class/power_supply", entryname,
                                     "present", NULL);
      if (g_file_test(path1, G_FILE_TEST_EXISTS)) {
        g_free(path1);
        battery_dir =
            g_build_filename("/sys/class/power_supply", entryname, NULL);
        break;
      }
      g_free(path1);
    }
...
void update_battery() {
  ...
  if (info.battery_life == 100) battery_state.state = BATTERY_FULL;
  ...
  if (energy_full > 0) new_percentage = (energy_now * 100) / energy_full;
...
int resize_battery(void* obj) {
  ...
  get_text_size2(bat1_font_desc, &bat_percentage_height_ink,
                 &bat_percentage_height, &bat_percentage_width,
                 panel->area.height, panel->area.width, buf_bat_percentage,
                 strlen(buf_bat_percentage));
  get_text_size2(bat2_font_desc, &bat_time_height_ink, &bat_time_height,
                 &bat_time_width, panel->area.height, panel->area.width,
                 buf_bat_time, strlen(buf_bat_time));

  if (panel_horizontal) {
    int new_size = (bat_percentage_width > bat_time_width)
                       ? bat_percentage_width
                       : bat_time_width;
    new_size +=
        (2 * battery->area.paddingxlr) + (2 * battery->area.bg->border.width);
    if (new_size > battery->area.width ||
        new_size < (battery->area.width - 2)) {
      // we try to limit the number of resize
      battery->area.width = new_size;
      battery->bat1_posy =
          (battery->area.height - bat_percentage_height - bat_time_height) / 2;
      battery->bat2_posy = battery->bat1_posy + bat_percentage_height;
      ret = 1;
    }
  } else {
    int new_size =
        bat_percentage_height + bat_time_height +
