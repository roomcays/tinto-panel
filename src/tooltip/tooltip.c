...
void tooltip_trigger_show(Area* area, Panel* p, XEvent* e) {
  ...
  if (!panel_horizontal) y -= height / 2;
  g_tooltip.panel = p;
...
void tooltip_show(void* arg) {
  ...
  Area* area;
  if (!panel_horizontal)
    my += height / 2; /* we adjusted y in tooltip_trigger_show, revert
                       * or we won't find the correct area anymore */
  point_T point = {mx, my};
...
void tooltip_adjust_geometry() {
  ...
  Panel* panel = g_tooltip.panel;
  int screen_width =
      server.monitor[panel->monitor].x + server.monitor[panel->monitor].width;
  int screen_height =
      server.monitor[panel->monitor].y + server.monitor[panel->monitor].height;
  ...
  if (x + width >
      server.monitor[panel->monitor].x + server.monitor[panel->monitor].width)
    x = server.monitor[panel->monitor].x +
        server.monitor[panel->monitor].width - width;
  if (y + height >
      server.monitor[panel->monitor].y + server.monitor[panel->monitor].height)
    y = server.monitor[panel->monitor].y +
        server.monitor[panel->monitor].height - height;
...
void start_show_timeout() {
  if (g_tooltip.timeout)
    change_timeout(g_tooltip.timeout, g_tooltip.show_timeout_msec, 0,
                   tooltip_show, 0);
  else
    g_tooltip.timeout =
        add_timeout(g_tooltip.show_timeout_msec, 0, tooltip_show, 0);
}

void start_hide_timeout() {
  if (g_tooltip.timeout)
    change_timeout(g_tooltip.timeout, g_tooltip.hide_timeout_msec, 0,
                   tooltip_hide, 0);
  else
    g_tooltip.timeout =
        add_timeout(g_tooltip.hide_timeout_msec, 0, tooltip_hide, 0);
}
