...
void get_action (char* event, int* action) {
  if (strcmp (event, "none") == 0) *action = NONE;
  else if (strcmp (event, "close") == 0) *action = CLOSE;
  else if (strcmp (event, "toggle") == 0) *action = TOGGLE;
  else if (strcmp (event, "iconify") == 0) *action = ICONIFY;
  else if (strcmp (event, "shade") == 0) *action = SHADE;
  else if (strcmp (event, "toggle_iconify") == 0) *action = TOGGLE_ICONIFY;
  else if (strcmp (event, "maximize_restore") == 0) *action = MAXIMIZE_RESTORE;
  else if (strcmp (event, "desktop_left") == 0) *action = DESKTOP_LEFT;
  else if (strcmp (event, "desktop_right") == 0) *action = DESKTOP_RIGHT;
  else if (strcmp (event, "next_task") == 0) *action = NEXT_TASK;
  else if (strcmp (event, "prev_task") == 0) *action = PREV_TASK;
}


int get_task_status(char* status) {
  if (strcmp(status, "active") == 0) return TASK_ACTIVE;
  else if (strcmp(status, "iconified") == 0) return TASK_ICONIFIED;
  else if (strcmp(status, "urgent") == 0) return TASK_URGENT;
...
void add_entry(char* key, char* value) {
  ...
  /* Panel */
    ...
    for (size_t j = 0, len = strlen(panel_items_order); j < len; ++j) {
      if (panel_items_order[j] == 'L') launcher_enabled = 1;
      else if (panel_items_order[j] == 'T') taskbar_enabled = 1;
    ...
    else if (strcmp (value1, "bottom") == 0) panel_position = BOTTOM;
    else panel_position = CENTER;

    if (!value2) panel_position |= CENTER;
    else if (strcmp(value2, "left") == 0) panel_position |= LEFT;
    else if (strcmp(value2, "right") == 0) panel_position |= RIGHT;
    else panel_position |= CENTER;

    if (!value3) panel_horizontal = 1;
    else if (strcmp(value3, "vertical") == 0) panel_horizontal = 0;
