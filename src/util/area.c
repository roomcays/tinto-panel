 ...
 * - size of a node/Area depend on the content (SIZE_BY_CONTENT objects) or on
 *the layout (SIZE_BY_LAYOUT objects)
 *
...
void size_by_content(Area* a) {
  ...
  GSList* l;
  for (l = a->list; l; l = l->next) size_by_content(l->data);
...
void size_by_layout(Area* a, int pos, int level) {
  ...
  for (l = a->list; l; l = l->next) {
    ...
    (child->size_mode == SIZE_BY_LAYOUT) ? "SIZE_BY_LAYOUT" :
    "SIZE_BY_CONTENT");*/
  ...
  if (a->on_changed) {
    // pos/size changed
    a->redraw = 1;
    if (a->_on_change_layout) a->_on_change_layout(a);
  }
}
...
void refresh(Area* a) {
  ...
  XCopyArea(server.dsp, a->pix, ((Panel*)a->panel)->temp_pmap, server.gc, 0, 0,
            a->width, a->height, a->posx, a->posy);

  // and then refresh child object
  GSList* l;
  for (l = a->list; l; l = l->next) refresh(l->data);
}
  ...
  if (panel_horizontal) {
    ...
    for (l = a->list; l; l = l->next) {
      ...
      if (child->on_screen && child->size_mode == SIZE_BY_LAYOUT) {
        ...
        if (child->width != old_width) child->on_changed = 1;
      }
    }
  } else {
    // detect free size for SIZE_BY_LAYOUT's Area
    ...
    for (l = a->list; l; l = l->next) {
      ...
      if (child->on_screen && child->size_mode == SIZE_BY_LAYOUT) {
        ...
        if (child->height != old_height) child->on_changed = 1;
      }
    }
  }
  return 0;
...
void set_redraw(Area* a) {
  ...
  GSList* l;
  for (l = a->list; l; l = l->next) set_redraw(l->data);
}
...
void draw(Area* a) {
  ...
  if (server.real_transparency) clear_pixmap(a->pix, 0, 0, a->width, a->height);
  ...
  if (a->_draw_foreground) a->_draw_foreground(a, c);
...
void draw_background(Area* a, cairo_t* c) {
  ...
  if (a->bg->border.width > 0 && a->bg->border.alpha > 0.0) {
    ...
    // il faut d'abord appliquer une rotation de 90 (et -180 si l'angle est
    superieur a  180)
    ...
    cairo_pattern_add_color_stop_rgba (linpat, 0, a->border.color[0],
    a->border.color[1], a->border.color[2], a->border.alpha);
    cairo_pattern_add_color_stop_rgba (linpat, 1, a->border.color[0],
...
void free_area(Area* a) {
  GSList* l0;
  for (l0 = a->list; l0; l0 = l0->next) free_area(l0->data);
