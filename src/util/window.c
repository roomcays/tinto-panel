...
int window_is_hidden(Window win) {
  ...
  at = server_get_property(win, server.atom._NET_WM_WINDOW_TYPE, XA_ATOM,
                           &count);
  for (i = 0; i < count; i++) {
...
GSList* server_get_name_of_desktop() {
  ...
  gchar* data_ptr, *ptr;
  data_ptr =
      server_get_property(server.root_win, server.atom._NET_DESKTOP_NAMES,
...
void get_text_size(PangoFontDescription* font, int* height_ink, int* height,
  ...
  cairo_surface_t* cs = cairo_xlib_surface_create(
      server.dsp, pmap, server.visual, panel_height, panel_height);
...
void get_text_size2(PangoFontDescription* font, int* height_ink, int* height,
                    int* width, int panel_height, int panel_with, char* text,
                    int len) {
  PangoRectangle rect_ink, rect;
  ...
  cairo_surface_t* cs = cairo_xlib_surface_create(
      server.dsp, pmap, server.visual, panel_height, panel_with);
  cairo_t* c = cairo_create(cs);
