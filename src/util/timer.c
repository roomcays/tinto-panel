...
*timeout in the list,
  * and callback_timeout_expired() only have to consider the timeouts as long as
*the expiration time
  ...
  while (it) {
    ...
    if (t2->interval_msec > 0) {
      if (t->interval_msec % t2->interval_msec == 0 ||
        ...
        if (multi_timeouts == 0) multi_timeouts = g_hash_table_new(0, 0);
...
int calc_multi_timeout_interval(multi_timeout_handler* mth) {
  ...
  while (it) {
    t = it->data;
    if (t->interval_msec < min_interval) min_interval = t->interval_msec;
    it = it->next;
  }
  return min_interval;
}
