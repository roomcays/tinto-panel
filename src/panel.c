...
void init_panel(void) {
  ...
  for (int i = 0 ; i < nb_panel ; ++i) {
    ...
    // catch some events
    XSetWindowAttributes att = { .colormap = server.colormap,
...
void set_panel_background(Panel* p) {
  ...
  if (server.real_transparency) {
    ...
    XFillRectangle(server.dsp, p->area.pix, server.gc, 0, 0,
...
void autohide_show(void* p) {
  ...
  if (panel_horizontal) {
    if (panel_position & TOP) {
      ...
      XMoveResizeWindow(server.dsp, panel->main_win, panel->posx,
                        panel->posy, panel->area.width, panel->area.height);
    }
  }
  else {
    if (panel_position & LEFT) {
...
void autohide_hide(void* p) {
  ...
  if (panel_horizontal) {
    if (panel_position & TOP) {
      ...
      XMoveResizeWindow(server.dsp, panel->main_win, panel->posx,
                        panel->posy+diff, panel->hidden_width,
                        panel->hidden_height);
    }
  }
  else {
    if (panel_position & LEFT) {
      ...
      XMoveResizeWindow(server.dsp, panel->main_win, panel->posx+diff,
                        panel->posy, panel->hidden_width, panel->hidden_height);
    }
  }
  panel_refresh = 1;
}
...
void autohide_trigger_show(Panel* p) {
  ...
  if (p->autohide_timeout) {
    ...
    p->autohide_timeout = add_timeout(panel_autohide_show_timeout, 0,
                                      autohide_show, p);
  }
}
...
void autohide_trigger_hide(Panel* p) {
  ...
  if (p->autohide_timeout) {
    ...
    p->autohide_timeout = add_timeout(panel_autohide_hide_timeout, 0,
                                      autohide_hide, p);
  }
}
