...
void draw_systray(void* obj, cairo_t* c) {
  if (server.real_transparency || systray.alpha != 100 ||
    ...
    render_background =
        XCreatePixmap(server.dsp, server.root_win, systray.area.width,
                      systray.area.height, server.depth);
...
int resize_systray(void* obj) {
  ...
  if (panel_horizontal) {
    ...
    systray.area.width = (2 * systray.area.bg->border.width) +
                         (2 * systray.area.paddingxlr) +
                         (sysbar->icon_size * sysbar->icons_per_row) +
                         ((sysbar->icons_per_row - 1) * systray.area.paddingx);
  } else {
    int width = sysbar->area.width - 2 * sysbar->area.bg->border.width -
                2 * sysbar->area.paddingy;
    ...
    sysbar->marging = width - (sysbar->icons_per_row - 1) *
                                  (sysbar->icon_size + sysbar->area.paddingx) -
                      sysbar->icon_size;
    sysbar->icons_per_column =
        count / sysbar->icons_per_row + (count % sysbar->icons_per_row != 0);
    systray.area.height =
        (2 * systray.area.bg->border.width) + (2 * systray.area.paddingxlr) +
...
void on_change_systray(void* obj) {
  // here, systray.area.posx/posy are defined by rendering engine. so we can
  // calculate position of tray icon.
  Systraybar* sysbar = obj;
  Panel* panel = sysbar->area.panel;
  int i, posx, posy;
  int start = panel->area.bg->border.width + panel->area.paddingy +
              systray.area.bg->border.width + systray.area.paddingy +
              sysbar->marging / 2;
  if (panel_horizontal) {
    posy = start;
...
gboolean add_icon(Window id) {
  ...
  parent_window =
      XCreateWindow(server.dsp, panel->main_win, 0, 0, 30, 30, 0, attr.depth,
...

  {
    ...
    ret = XGetWindowProperty(server.dsp, id, server.atom._XEMBED_INFO, 0, 2,
  ...
  if (systray.area.on_screen == 0) show(&systray.area);
...
void remove_icon(TrayWindow* traywin) {
  ...
  error = FALSE;
  old = XSetErrorHandler(window_error_handler);
  if (!traywin->hide) XUnmapWindow(server.dsp, traywin->tray_id);
...
void systray_render_icon_now(void* t) {
  ...
  XCopyArea(server.dsp, render_background, systray.area.pix, server.gc,
            ...
            traywin->width, traywin->height, traywin->x - systray.area.posx,
  ...
  if (traywin->damage) XDamageSubtract(server.dsp, traywin->damage, None, None);
  XFlush(server.dsp);
}
  ...
  if (server.real_transparency || systray.alpha != 100 ||
    ...
    if (traywin->render_timeout == 0)
      traywin->render_timeout =
          add_timeout(50, 0, systray_render_icon_now, traywin);
  } else {
    ...
    //			XCopyArea(server.dsp, panel->temp_pmap, pix, server.gc, traywin->x,
    //traywin->y, traywin->width, traywin->height, 0, 0);
